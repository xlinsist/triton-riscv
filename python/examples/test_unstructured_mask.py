...
def test_unstructured_mask_2d(device):
    ...
    input = torch.arange(2, 2 + (m * n), device=device, dtype=torch.float32).reshape(
        m, n
    )
    ...
    res = unstructured_mask_2d_kernel[1,](
        input, output, mask_m.to(torch.int8), mask_n.to(torch.int8), m, n, M, N
    )
    tts_code = run_triton_to_structured(res.asm["ttir"])
    assert (
        tts_code.count("tts.make_gather_scatter_tptr") == 2
    ), "Expected exactly two calls to tts.make_gather_scatter_tptr"
...
@triton.jit
def unstructured_mask_3d_kernel(
    in_ptr,
    out_ptr,
    mask_m_ptr,
    mask_n_ptr,
...
def test_unstructured_mask_3d(device):
    ...
    N = triton.next_power_of_2(n)
    input = torch.arange(
        2, 2 + (b * m * n), device=device, dtype=torch.float32
    ).reshape(b, m, n)
    output = torch.full_like(input, -1)
    mask_m = torch.tensor([1, 0, 1, 0, 1, 0], device=device, dtype=torch.bool)
    ...
    res = unstructured_mask_3d_kernel[1,](
        input,
        output,
        mask_m.to(torch.int8),
...
@triton.jit
def unstructured_mask_2d_non_continuous_load_kernel(
    in_ptr,
    out_ptr,
    index_m_ptr,
    im,
...
def test_unstructured_mask_2d_non_continuous_load(device):
    ...
    res = unstructured_mask_2d_non_continuous_load_kernel[1,](
        input, output, index_m, len(index_m), I_M, N, m, n, stride_m, stride_n
    )
    tts_code = run_triton_to_structured(res.asm["ttir"])
    assert (
        tts_code.count("tts.make_gather_scatter_tptr") == 1
    ), "Expected exactly one call to tts.make_gather_scatter_tptr"

    expected_output = torch.full(
        (len(index_m), n), -2, device=device, dtype=torch.float32
    )
    mask_m = index_m < m
...
@triton.jit
def unstructured_mask_2d_non_continuous_store_kernel(
    in_ptr,
    out_ptr,
    index_n_ptr,
    i_n,
...
def test_unstructured_mask_2d_non_continuous_store(device):
    ...
    index_n = torch.tensor(
        [10, 20, 15, 1, 3, 5, 7, 0, 2, 4, 6, 8], device=device, dtype=torch.int32
    )
    ...
    res = unstructured_mask_2d_non_continuous_store_kernel[1,](
        input, output, index_n, len(index_n), I_N, M, m, n, stride_m, stride_n
    )
    tts_code = run_triton_to_structured(res.asm["ttir"])
    assert (
        tts_code.count("tts.make_gather_scatter_tptr") == 1
...
@triton.jit
def unstructured_mask_3d_non_continuous_load_kernel(
    in_ptr,
    out_ptr,
    index_m_ptr,
    im,
    ...
    out_ptrs = (
        out_ptr
        + offs_b[:, None, None] * o_stride_b
        + offs_m[None, :, None] * o_stride_m
        + offs_n[None, None, :] * o_stride_n
    )
    tl.store(
        out_ptrs,
        v,
...
def test_unstructured_mask_3d_non_continuous_load(device):
    ...
    res = unstructured_mask_3d_non_continuous_load_kernel[1,](
        input,
        output,
        index_m,
        len(index_m),
    ...
    expected_output = torch.full(
        (b, len(index_m), n), -2, device=device, dtype=torch.float32
    )
    mask_m = index_m < m
    index_m = index_m[mask_m]
    expected_output[:, : len(index_m), :] = input[:, index_m, :]
...
@triton.jit
def unstructured_mask_3d_non_continuous_store_kernel(
    in_ptr,
    out_ptr,
    index_n_ptr,
    i_n,
...
def test_unstructured_mask_3d_non_continuous_store(device):
    ...
    res = unstructured_mask_3d_non_continuous_store_kernel[1,](
        input,
        output,
        index_n,
        len(index_n),
...
@triton.jit
def unstructured_mask_and_structured_mask_kernel(
    in_ptr,
    out_ptr,
    m,
    n,
...
def test_unstructured_mask_and_structured_mask(device):
    ...
    N = triton.next_power_of_2(n)
    input = torch.arange(
        2, 2 + (n_pids * m * n), device=device, dtype=torch.float32
    ).reshape(n_pids, m, n)
    output = torch.full_like(input, -1)
    ...
    res = unstructured_mask_and_structured_mask_kernel[n_pids,](
        input, output, m, n, stride_pid, stride_m, stride_n, M, N
    )
    tts_code = run_triton_to_structured(res.asm["ttir"])
    assert (
        tts_code.count("tts.make_gather_scatter_tptr") == 2
    ...
    expected_output = unstructured_mask_and_structured_mask_torch(
        input, n_pids, m, n, M, N
    )
